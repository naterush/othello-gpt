...
def validate_probe(othello_gpt, linear_probe, valid_games_int, valid_games_str, config):
    """
    ...
    with torch.no_grad():
        for i in range(0, num_games, batch_size):
            batch_games_int = valid_games_int[i:i+batch_size].to(device)
            batch_games_str = valid_games_str[i:i+batch_size]
            
            # Get state stack and one hot encoding
            state_stack = build_state_stack(batch_games_str).to(device)
...
def train_with_config():
    ...
    def train(config=None):
        with wandb.init(config=config) as run:
            ...
            # Get data
            board_seqs_int, board_seqs_string, board_seqs_int_validation, board_seqs_string_validation = get_board_seqs_int_and_str(
            ...
            for epoch in range(config.epochs):
                ...
                for _ in range(config.probe_steps_per_epoch):
                    ...
                    games_str_batch = board_seqs_string[batch_indices]
                    state_stack = build_state_stack(games_str_batch).to(device)
                    state_stack = state_stack[:, pos_start:pos_end, :, :]
                    state_stack_one_hot = one_hot_function(state_stack)
...
sweep_config = {
    'method': 'random',
    'metric': {'name': 'othello_accuracy', 'goal': 'maximize'},
    'parameters': {
        'probe_lr': {'values': [1e-4, 5e-4, 1e-3]},
        'model_lr': {'values': [1e-4, 5e-4, 1e-3]},
        'probe_steps_per_epoch': {'values': [1, 10, 20]},
        'model_steps_per_epoch': {'values': [1, 10, 20]},
